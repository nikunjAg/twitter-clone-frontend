...
const getBookmarkedPostsStart = (state, action) => {
	return updatedObject(state, { posts: [], loading: true });
};

const getBookmarkedPostsSuccess = (state, action) => {
	return updatedObject(state, { posts: action.posts, loading: false });
};

const getBookmarkedPostsFail = (state, action) => {
...
const reducer = (state = initialState, action) => {
	switch (action.type) {
		case actionTypes.GET_BOOKMARKED_POSTS_START:
			return getBookmarkedPostsStart(state, action);
		case actionTypes.GET_BOOKMARKED_POSTS_SUCCESS:
			return getBookmarkedPostsSuccess(state, action);
		case actionTypes.GET_BOOKMARKED_POSTS_FAIL:
			return getBookmarkedPostsFail(state, action);
